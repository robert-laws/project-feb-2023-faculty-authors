...
export const PublicationsUnified = () => {
  const {
    publications,
  ...
  useEffect(() => {
    if (countList.length > 0 && (filtersTouched || query.length > 0)) {
  ...
  }, [countList, filtersTouched, query]);
  ...
  useEffect(() => {
    ...
    const applyFilters = (filterArray) => {
      let filteredPublicationsAll = [];
      if (searchResults.length > 0) {
        filteredPublicationsAll = searchResults;
      } else {
        filteredPublicationsAll = publications;
      }
      ...
      for (let i = 0; i < filterArray.length; i++) {
        ...
        if (filters.length > 0) {
          filteredPublicationsAll = filteredPublications.filter((publication) =>
            filters.includes(publication[list])
          );
        }
      }

      return filteredPublicationsAll;
    };
